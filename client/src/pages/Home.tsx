...
const faqs = [
  ...
  },
  {
    ...
    answer: (
      <>
        Você pode enviar uma mensagem pelo <a className="inline-link" href={WHATSAPP_URL} data-umami-event="whatsapp-click" data-umami-event-placement="faq-answer">WhatsApp</a> e contar um pouco sobre a sua operação. A equipe retorna para combinar o próximo passo.
      </>
    ),
  },
];
    ...
    <div className="site-shell">
      <header className="site-header">
        <div className="nav-wrap">
          ...
          <nav className={`main-nav ${menuOpen ? "is-open" : ""}`} aria-label="Navegação principal">
            ...
            <a className="nav-cta" href={WHATSAPP_URL} data-umami-event="whatsapp-click" data-umami-event-placement="navigation" onClick={() => setMenuOpen(false)}>
      ...
      <main id="top">
          ...
          <div className="hero-content page-width">
            <div className="hero-copy">
              ...
              <div className="hero-actions">
                <a className="button button-lime" href={WHATSAPP_URL} data-umami-event="whatsapp-click" data-umami-event-placement="hero">solicitar demonstração <ArrowUpRight size={17} /></a>
        ...
        <section className="support-section">
          <div className="page-width support-grid"><div className="support-label"><SectionLabel index="Z/07" light>comece com contexto</SectionLabel><span className="support-mark">Z/</span></div><div className="support-copy"><h2>Implantar é mais simples quando alguém entende <em>o seu balcão.</em></h2><p>A conversa começa pelo seu cenário: como você vende, como organiza a equipe e onde hoje existe retrabalho. A MC Infra TI orienta o próximo passo com uma visão prática de implantação e suporte.</p><div className="support-list"><span><ShieldCheck size={18} /> segurança e permissões por operador</span><span><MessageCircle size={18} /> atendimento técnico para a sua rotina</span><span><Sparkles size={18} /> evolução guiada pela operação</span></div><a className="button button-dark" href={WHATSAPP_URL} data-umami-event="whatsapp-click" data-umami-event-placement="support">falar com a MC Infra TI <ArrowUpRight size={17} /></a></div></div>
        </section>

        <section className="faq-section page-width" id="faq"><div className="faq-intro"><SectionLabel index="Z/08">perguntas de operação</SectionLabel><h2>Antes de começar,<br /><span>vamos simplificar.</span></h2><p>Se sua dúvida não estiver aqui, envie uma mensagem. A melhor resposta depende do seu fluxo.</p><a className="inline-link" href={WHATSAPP_URL} data-umami-event="whatsapp-click" data-umami-event-placement="faq-section">falar pelo WhatsApp <ArrowUpRight size={15} /></a></div><div className="faq-list">{faqs.map((faq, index) => { const isOpen = openFaq === index; return <div className={`faq-item ${isOpen ? "is-open" : ""}`} key={faq.question}><button type="button" aria-expanded={isOpen} onClick={() => setOpenFaq(isOpen ? -1 : index)}><span>0{index + 1}</span><strong>{faq.question}</strong><ChevronDown size={19} /></button><div className="faq-answer"><p>{faq.answer}</p></div></div>; })}</div></section>

        <section className="final-cta dark-section"><div className="page-width final-cta-inner"><div className="cta-sticker">Z/ próximo passo</div><SectionLabel index="Z/09" light>quando o caixa pede mais clareza</SectionLabel><h2>Seu próximo turno pode começar<br /><em>mais organizado.</em></h2><p>Conte para a MC Infra TI como o seu negócio funciona. A gente mostra onde o Zelda PDV Food pode entrar.</p><a className="button button-lime" href={WHATSAPP_URL} data-umami-event="whatsapp-click" data-umami-event-placement="final-cta">solicitar uma demonstração <ArrowUpRight size={18} /></a></div></section>
      </main>

      <footer className="site-footer"><div className="page-width footer-top"><div className="footer-brand"><a href="#top"><BrandMark size="md" /></a><p>Gestão e vendas para operações food service que precisam de ritmo, clareza e controle.</p><span className="footer-location">MC INFRA TI · BRASIL</span></div><div className="footer-col"><span className="footer-heading">navegação</span><a href="#visao-geral">Visão geral</a><a href="#recursos">Recursos</a><a href="#segmentos">Para quem é</a><a href="#faq">Dúvidas</a></div><div className="footer-col"><span className="footer-heading">contato</span><a href={WHATSAPP_URL} data-umami-event="whatsapp-click" data-umami-event-placement="footer">WhatsApp</a><a href={`mailto:${CONTACT_EMAIL}`}>{CONTACT_EMAIL}</a><span>Atendimento MC Infra TI</span></div><div className="footer-col"><span className="footer-heading">legal</span><Link href="/politica-de-privacidade">Política de privacidade</Link><Link href="/termos-de-uso">Termos de uso</Link><Link href="/politica-de-cookies">Política de cookies</Link><Link href="/direitos-do-titular">Direitos do titular</Link></div></div><div className="page-width footer-bottom"><span>© 2026 MC Infra TI. Todos os direitos reservados.</span><span>ZELDA PDV FOOD / SISTEMA DE GESTÃO E VENDAS</span></div></footer>

      {cookieVisible && <aside className="cookie-banner" aria-label="Aviso de cookies"><div><span className="mono-label">privacidade em primeiro lugar</span><p>Usamos cookies para melhorar a experiência e entender o uso da página. Consulte a <a href="/politica-de-privacidade">Política de Privacidade</a>.</p></div><button className="cookie-button" type="button" onClick={acceptCookies}>entendi <Check size={16} /></button></aside>}
    </div>
  );
}